...
# gien: MCP entrypoints live under color_master; qbrain/utils/jax_test resolve from repo ``cor/``.
...
def _patch_streamable_http_accept_wildcard() -> None:
    ...
    try:
        ...
        print(f"Err color_master.mcp_server.mcp_routes::_patch_streamable_http_accept_wildcard | handler_line=45 | {type(exc).__name__}: {exc}")
        print(f"[exception] color_master.mcp_server.mcp_routes._patch_streamable_http_accept_wildcard: {exc}")
...
def _jsonable(value: Any) -> Any:
    if hasattr(value, "model_dump"):
        try:
            ...
            print("Err color_master.mcp_server.mcp_routes::_jsonable | handler_line=177 | Exception handler triggered")
            print("[exception] color_master.mcp_server.mcp_routes._jsonable: caught Exception")
...
def _maybe_run_windows_bootstrap_checks(*, server: FastMCP, host: str, port: int, path: str) -> None:
    ...
    try:
        ...
        print(f"Err color_master.mcp_server.mcp_routes::_maybe_run_windows_bootstrap_checks | handler_line=213 | {type(exc).__name__}: {exc}")
        print(f"[exception] color_master.mcp_server.mcp_routes._maybe_run_windows_bootstrap_checks: {exc}")
    ...
    if _to_bool(os.getenv("MCP_RUN_BOOTSTRAP_TESTER"), False):
        try:
            ...
            print(f"Err color_master.mcp_server.mcp_routes::_maybe_run_windows_bootstrap_checks | handler_line=224 | {type(exc).__name__}: {exc}")
            print(f"[exception] color_master.mcp_server.mcp_routes._maybe_run_windows_bootstrap_checks: {exc}")
...
def _resolve_type_hints(service_method: Callable[..., Any], method_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Err color_master.mcp_server.mcp_routes::_resolve_type_hints | handler_line=301 | {type(exc).__name__}: {exc}")
        print(f"[exception] color_master.mcp_server.mcp_routes._resolve_type_hints: {exc}")
...
def _read_runtime_config() -> Dict[str, Any]:
    ...
    port = int(os.getenv("PORT") or os.getenv("MCP_PORT") or str(DEFAULT_PORT))
