"""FeatureEncoder: linear projects per shaped grid; create_in_features pads/trims tail to `in_features` to avoid dot_general mismatch."""
...
class FeatureEncoder(eqx.Module):
    ...
    def save_features(self, batch_in_features):
        # gien: timestep feature list for serialization (`gnn.calc_batch` after `save_t_step`)
        try:
            self.in_ts.append(list(batch_in_features))
        except Exception as e:
            print("Err save_features:", e)
    ...
    def build_single_linear(self, ilen, eq_idx, variation_idx):
        linear = eqx.nn.Linear(
            in_features=len(ilen),
            out_features=self.d_model,
            key=self.rngs,
        )
        # append linear for each shape for each variation to each eq-space
            ...
            for var_idx, (grids_for_var, linear_row) in enumerate(
                ...
                for grid, linear in zip(grids_for_var, linear_row):
                    ...
                    if var_idx < len(self.in_f_store[eq_idx]):
                        # gien: store row-wise slices when vmap returns a batch matrix; scalars append as one item
