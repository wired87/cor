...
def _load_cfg(args: argparse.Namespace) -> dict:
    if args.cfg_string is not None:
        try:
            ...
            print(f"Err core.jax_test.grid.__main__::_load_cfg | handler_line=37 | {type(e).__name__}: {e}")
            print(f"[exception] core.jax_test.grid.__main__._load_cfg: {e}")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Err core.jax_test.grid.__main__::main | handler_line=61 | {type(e).__name__}: {e}")
        print(f"[exception] core.jax_test.grid.__main__.main: {e}")
