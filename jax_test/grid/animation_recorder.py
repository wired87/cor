...
class GridAnimationRecorder:
    ...
    def save_frame(self, step: int, data: np.ndarray) -> None:
        ...
        try:
            ...
            print(f"Err cor.jax_test.grid.animation_recorder::GridAnimationRecorder.save_frame | handler_line=72 | {type(e).__name__}: {e}")
            print(f"[exception] cor.jax_test.grid.animation_recorder.GridAnimationRecorder.save_frame: {e}")
    ...
    def finish(self) -> str | None:
        ...
        try:
            ...
            print(f"Err cor.jax_test.grid.animation_recorder::GridAnimationRecorder.finish | handler_line=102 | {type(e).__name__}: {e}")
            print(f"[exception] cor.jax_test.grid.animation_recorder.GridAnimationRecorder.finish: {e}")
            ...
            for p in self._frame_paths:
                try:
                    ...
                    print("Err cor.jax_test.grid.animation_recorder::GridAnimationRecorder.finish | handler_line=110 | Exception handler triggered")
                    print("[exception] cor.jax_test.grid.animation_recorder.GridAnimationRecorder.finish: caught Exception")
                    pass

    def _save_to_envs(self, animation_path: str) -> None:
        """Local file path only — no DB update."""
        print(f"[GridAnimationRecorder] animation saved locally: {animation_path}")
