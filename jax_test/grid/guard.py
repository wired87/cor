"""
Grid Guard: consumes cfg (components from cor.guard.converter) and runs the expected workflow.
...
class Guard:
    ...
    Cfg is the output of cor.guard.Guard.converter() (e.g. from test_out.json).
    """
    ...
    def _decode_db(self, cfg: dict) -> np.ndarray | None:
        ...
        try:
            ...
            print(f"Err cor.jax_test.grid.guard::Guard._decode_db | handler_line=51 | {type(e).__name__}: {e}")
            print(f"[exception] cor.jax_test.grid.guard.Guard._decode_db: {e}")
