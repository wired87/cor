...
from qfu.all_subs import ALL_SUBS, FERMIONS
...
class RuntimeUtilsCreator(
    ...
    def g_neighbors_gg(self, attrs):
        try:
            ...
            for nnid in g_neighbors:
                nattrs = self.g.get_node(id=nnid)
            ...
            print(f"Err core.qfu.runtime_utils_creator::RuntimeUtilsCreator.g_neighbors_gg | handler_line=89 | {type(e).__name__}: {e}")
            print(f"[exception] core.qfu.runtime_utils_creator.RuntimeUtilsCreator.g_neighbors_gg: {e}")
    ...
    def neighbor_f(self, attrs):
        ...
        try:
            ...
            print(f"Err core.qfu.runtime_utils_creator::RuntimeUtilsCreator.neighbor_f | handler_line=115 | {type(e).__name__}: {e}")
            print(f"[exception] core.qfu.runtime_utils_creator.RuntimeUtilsCreator.neighbor_f: {e}")
    ...
    def g_neighbors(self, attrs):
        ...
        try:
            ...
            # todo save new admin_data structures
            filtered = [[], [], []]
            ...
            print(f"Err core.qfu.runtime_utils_creator::RuntimeUtilsCreator.g_neighbors | handler_line=152 | {type(e).__name__}: {e}")
            print(f"[exception] core.qfu.runtime_utils_creator.RuntimeUtilsCreator.g_neighbors: {e}")
            print("Err g_neighbors:", e)

    # todo coding sys based on components - not single code lines
    def h_neighbor(self, attrs):
        id=attrs["id"]
        return self.g.get_node(id=f"PHI__px{nid.split('_px')[-1]}")
    ...
    def get_step_eq_val(self, attrs, method_key):
        ...
        try:
            ...
            print(f"Err core.qfu.runtime_utils_creator::RuntimeUtilsCreator.get_step_eq_val | handler_line=201 | {type(e).__name__}: {e}")
            print(f"[exception] core.qfu.runtime_utils_creator.RuntimeUtilsCreator.get_step_eq_val: {e}")
    ...
    def create_utils_stack_all_subs(self, pxu, attrs) -> List[dict]:
        try:
            ...
            print(f"Err core.qfu.runtime_utils_creator::RuntimeUtilsCreator.create_utils_stack_all_subs | handler_line=248 | {type(e).__name__}: {e}")
            print(f"[exception] core.qfu.runtime_utils_creator.RuntimeUtilsCreator.create_utils_stack_all_subs: {e}")
        ...
        for key in params:
            ...
            if any(v is None for v in vals):
                try:
                    ...
                    node = ray.get(uw.get_node.remote(id=key))
