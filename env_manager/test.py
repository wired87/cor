...
from env_manager.env_lib import handle_set_env, handle_get_envs_user, handle_del_env
from firegraph import flatten_payload
...
def test_env_manager():
    """
    Test env manager for the surrounding workflow.

    Workflow:
    1. Starts from the current object state and local workflow context.
    2. Builds intermediate state such as `user_id`, `env_id`, `env_data` before applying the main logic.
    3. Branches on validation or runtime state to choose the next workflow path.
    4. Delegates side effects or helper work through `print()`, `handle_set_env()`, `resp.get('data', {}).get()`.
    ...
    try:
        ...
        print(f"Err core.qbrain.core.env_manager.test::test_env_manager | handler_line=34 | {type(e).__name__}: {e}")
        print(f"[exception] core.qbrain.core.env_manager.test.test_env_manager: {e}")
