    ...
    def working_queue(
        ...
        while True:
            try:
                ...
                if task_type == 'firebase_push':
                    ...
                    if db_path and data_to_push:
                        ...
                        try:
                            ...
                            print(f"Err cor.utils.queue_handler::QueueHandler.working_queue | handler_line=66 | {type(e).__name__}: {e}")
                            print(f"[exception] cor.utils.queue_handler.QueueHandler.working_queue: {e}")
                ...
                print("Err cor.utils.queue_handler::QueueHandler.working_queue | handler_line=88 | queue.Empty handler triggered")
                print("[exception] cor.utils.queue_handler.QueueHandler.working_queue: caught queue.Empty")
                pass
            except Exception as e:
                print(f"Err cor.utils.queue_handler::QueueHandler.working_queue | handler_line=93 | {type(e).__name__}: {e}")
                print(f"[exception] cor.utils.queue_handler.QueueHandler.working_queue: {e}")
                print(
