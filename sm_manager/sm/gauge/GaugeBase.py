...
class GaugeBase(
    ...
    def __init__(
        ...
        Workflow:
        1. Reads and normalizes the incoming inputs, including `env`, `g_utils`.
        2. Delegates side effects or helper work through `RayValidator.__init__()`.
    ...
    def main(
        ...
        2. Delegates side effects or helper work through `self.gf_coupling()`, `self.gg_coupling()`, `self._dmu_field_value()`.
    ...
    def gg_coupling(self, attrs, g_neighbors):
        """
        Gg coupling for the GaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs`, `g_neighbors`.
        2. Builds intermediate state such as `g_self`, `field_key`, `field_v_self` before applying the main logic.
        3. Iterates through the available items and applies the same workflow rules consistently.
        4. Delegates side effects or helper work through `attrs.get()`, `g_neighbors.items()`, `jnp.array()`.
        ...
        - `g_neighbors`: Caller-supplied value used during processing.
    ...
    def _dmu_fmunu(
        ...
        # dmuF(mu_nu)
        """
        Dmu fmunu for the GaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs`, `neighbor_pm_val_fmunu`.
        2. Builds intermediate state such as `field_key`, `dmufmunu` before applying the main logic.
        3. Delegates side effects or helper work through `self.call()`.
    ...
    def _dmu_field_value(
        ...
        """
        Dmu field value for the GaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs`, `neighbor_pm_val_same_type`.
        2. Builds intermediate state such as `field_key`, `d_field_value_key`, `dmu` before applying the main logic.
        3. Delegates side effects or helper work through `self.call()`.
        4. Finishes by updating state, triggering side effects, or completing the workflow without a direct return value.
