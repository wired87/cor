...
class GaugeBase(
    ...
    def __init__(self):
        """
        Initialize the GaugeBase instance state.

        Workflow:
        1. Starts from the current object state and local workflow context.
        2. Delegates side effects or helper work through `GaugeUtils.__init__()`.
    ...
    def main(
        ...
        """
        Run the main GaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs`, `neighbor_pm_val_same_type`, `neighbor_pm_val_fmunu`, `f_neighbors`.
        2. Delegates side effects or helper work through `self.coupling()`, `self._dmu_field_value()`, `self._F_mu_nu()`.
        3. Returns the assembled result to the caller.

        Inputs:
        - `attrs`: Caller-supplied value used during processing. Expected type: `dict`.
        - `neighbor_pm_val_same_type`: Caller-supplied value used during processing.
        - `neighbor_pm_val_fmunu`: Caller-supplied value used during processing.
        - `f_neighbors`: Caller-supplied value used during processing. Expected type: `dict[str, dict]`.
        - `g_neighbors`: Caller-supplied value used during processing. Expected type: `dict[str, dict]`.
    ...
    def tripple_id_list_powerset(self, neighbors):
        """
        Tripple id list powerset for the GaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `neighbors`.
        2. Builds intermediate state such as `id_set` before applying the main logic.
        3. Iterates through the available items and applies the same workflow rules consistently.
        4. Delegates side effects or helper work through `neighbors.items()`, `list()`, `id_set.append()`.
        5. Returns the assembled result to the caller.

        Inputs:
        - `neighbors`: Caller-supplied value used during processing.
...
class TGaugeBase(
    ...
    def __init__(
        ...
        """
        Initialize the TGaugeBase instance state.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `ntype`, `env`, `g_utils`.
        2. Delegates side effects or helper work through `RayValidator.__init__()`, `GaugeUtils.__init__()`, `self.get_gauge_field_symbol()`.
        3. Finishes by updating state, triggering side effects, or completing the workflow without a direct return value.

        Inputs:
        - `ntype`: Caller-supplied value used during processing.
        - `env`: Caller-supplied value used during processing.
        - `g_utils`: Caller-supplied value used during processing.
    ...
    def main(
        ...
        # args
        """
        Run the main TGaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `env`, `attrs`, `all_subs`, `neighbor_pm_val_same_type`.
        2. Builds intermediate state such as `ntype`, `field_key`, `prev_key` before applying the main logic.
        3. Branches on validation or runtime state to choose the next workflow path.
        4. Delegates side effects or helper work through `self.restore_selfdict()`, `getattr()`, `self.attrs.get()`.
        5. Returns the assembled result to the caller.

        Inputs:
        - `env`: Caller-supplied value used during processing.
        - `attrs`: Caller-supplied value used during processing.
        - `all_subs`: Caller-supplied value used during processing.
        - `neighbor_pm_val_same_type`: Caller-supplied value used during processing.
        - `neighbor_pm_val_fmunu`: Caller-supplied value used during processing.
    ...
    def _dmu_field_value(self, field_key, d_field_value_key):
        """
        Dmu field value for the TGaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `field_key`, `d_field_value_key`.
        2. Builds intermediate state such as `dmu` before applying the main logic.
        3. Delegates side effects or helper work through `self.call()`, `setattr()`.
        ...
        - `d_field_value_key`: Caller-supplied value used during processing.
    ...
    def _dmu_fmunu(self, field_key):
        # dmuF(mu_nu)
        """
        Dmu fmunu for the TGaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `field_key`.
        2. Builds intermediate state such as `dmu` before applying the main logic.
        3. Delegates side effects or helper work through `self.call()`, `setattr()`.
    ...
    def _update_gauge_field(self, field_value, field_key, nid,  F_mu_nu, j_nu, f_abc, dmu_F_mu_nu, ntype):
        ...
        """
        Update gauge field for the TGaugeBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `field_value`, `field_key`, `nid`, `F_mu_nu`.
        2. Builds intermediate state such as `new_fv` before applying the main logic.
        3. Delegates side effects or helper work through `self.update_gauge_field()`, `print()`, `setattr()`.
        4. Finishes by updating state, triggering side effects, or completing the workflow without a direct return value.
        ...
        - `F_mu_nu`: Caller-supplied value used during processing.
        - `j_nu`: Caller-supplied value used during processing.
        - `f_abc`: Caller-supplied value used during processing.
        - `dmu_F_mu_nu`: Caller-supplied value used during processing.
        - `ntype`: Caller-supplied value used during processing.
    ...
    def tripple_id_list_powerset(self, neighbors):
        """
        Tripple id list powerset for the TGaugeBase workflow.
