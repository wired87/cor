...
class HiggsBase(
    ...
    def __init__(self, env):
        ...
        2. Delegates side effects or helper work through `HiggsUtils.__init__()`, `FieldUtils.__init__()`, `jit()`.
    ...
    # but the cor logic is now in the JAX-compiled wrapper.
    ...
    def list_to_soa(self, attrs_list: List[dict]) -> Dict[str, jnp.ndarray]:
        ...
        """
        List to soa for the HiggsBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs_list`.
        2. Delegates side effects or helper work through `jnp.array()`.
        3. Returns the assembled result to the caller.

        Inputs:
        - `attrs_list`: Caller-supplied value used during processing. Expected type: `List[dict]`.

        Returns:
        - Returns `Dict[str, jnp.ndarray]` to the caller.
        """
    ...
    def soa_to_list(self, attrs_soa: Dict[str, jnp.ndarray]) -> List[dict]:
        ...
        """
        Soa to list for the HiggsBase workflow.

        Workflow:
        1. Reads and normalizes the incoming inputs, including `attrs_soa`.
        2. Builds intermediate state such as `num_fields`, `attrs_list` before applying the main logic.
        3. Iterates through the available items and applies the same workflow rules consistently.
        4. Delegates side effects or helper work through `len()`, `range()`, `next()`.
        5. Returns the assembled result to the caller.

        Inputs:
        - `attrs_soa`: Caller-supplied value used during processing. Expected type: `Dict[str, jnp.ndarray]`.

        Returns:
        - Returns `List[dict]` to the caller.
        """
